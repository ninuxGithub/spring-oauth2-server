			...
			<form:form action="${pageContext.request.contextPath}/submitUpdate"
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<p class="help-block">client_id必须输入,且必须唯一,长度至少5位;
							在实际应用中的另一个名称叫appKey,与client_id是同一个概念.</p>
					</div>
				</div>
				<div class="form-group">
						...
						<div class="col-sm-10">
							<c:forEach var="tag" items="${tags}">
								<label class="checkbox-inline"> <form:checkbox
										path="privileges" value="${tag.id}" /> ${tag.id}
								</label>
							</c:forEach>
							<p class="help-block">Select Privilege(s). privilege is form
								security.xml</p>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<label class="checkbox-inline"> <input type="checkbox"
							...
							${fun:containsIgnoreCase(formDto.authorizedGrantTypes, 'password') ?'checked':''} />
							password
						</label> <label class="checkbox-inline"> <input type="checkbox"
						...
						<p class="help-block">
							至少勾选一项grant_type(s), 且不能只单独勾选
							<code>refresh_token</code>
							, 若需更多帮助请访问 <a href="https://andaily.com/blog/?p=103"
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<p class="help-block">
							...
							包括<em>authorization_code</em>或<em>implicit</em>,
							则必须输入redirect_uri
						</p>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<form:select path="authorities" id="authorities"
							...
							<form:option value="ROLE_USER,ROLE_UNITY,ROLE_MOBILE,ROLE_ADMIN">ROLE_UNITY,ROLE_MOBILE,ROLE_ADMIN</form:option>
						</form:select>

						<p class="help-block">
							指定客户端所拥有的Spring Security的权限值,可选; 若
							<code>grant_type</code>
							为<em>implicit</em>或<em>client_credentials</em>则必须选择authorities,
				...
				<div ng-show="visible">
					<div class="form-group">
						...
						<div class="col-sm-10">
							<form:input type="number" class="form-control"
								path="accessTokenValidity" id="accessTokenValidity"
								placeholder="access_token_validity" />

							<p class="help-block">设定客户端的access_token的有效时间值(单位:秒),可选,
								若不设定值则使用默认的有效时间值(60 * 60 * 12, 12小时); 若设定则必须是大于0的整数值</p>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<form:input type="number" class="form-control"
								path="refreshTokenValidity" id="refreshTokenValidity"
								placeholder="refresh_token_validity" />

							<p class="help-block">设定客户端的refresh_token的有效时间值(单位:秒),可选,
								若不设定值则使用默认的有效时间值(60 * 60 * 24 * 30, 30天); 若设定则必须是大于0的整数值</p>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<form:input type="text" class="form-control"
								path="additionalInformation" id="additionalInformation"
